 ...
 */
public class BatchSubmitPane extends BasicBeanPane<SubmitMain> implements Level {
    ...
    public BatchSubmitPane.KeyColumnNameValueTable keyColumnValuesTable;
     ...
     */
    private BatchSubmitPane.ColumnName[] currentColumnNames = null;
     ...
     */
    public BatchSubmitPane() {
        this(ValueEditorPaneFactory.extendedCellGroupEditors());
    }

    public BatchSubmitPane(Editor<?>[] v_Types) {
        ...
        keyColumnValuesTable = new BatchSubmitPane.KeyColumnNameValueTable();
        ...
        if (actions != null && actions.length > 0) {
            ...
            helpButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    BasicDialog dlg = formatExplanation.showMediumWindow(SwingUtilities.getWindowAncestor(BatchSubmitPane.this),
    ...
    private UIButton addEventButton() {
        ...
        addSubmitEventButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                BasicDialog dialog = jobsPane.showWindow(SwingUtilities.getWindowAncestor(BatchSubmitPane.this), new DialogActionAdapter() {
                    @Override
                    public void doOk() {
                        super.doOk();
    ...
    protected UpdateAction[] getActions() {
        return new UpdateAction[]{new BatchSubmitPane.SmartAddFieldsAction(), new BatchSubmitPane.AddFieldAction(), new BatchSubmitPane.RemoveFieldAction()};
    }
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            final CheckBoxList list = new CheckBoxList(currentColumnNames(), CheckBoxList.SelectedState.ALL, Inter.getLocText("FR-Designer_Chart_Field_Name")) {
                public String value2Text(Object value) {
                    if (value instanceof BatchSubmitPane.ColumnName) {
                        return ((BatchSubmitPane.ColumnName) value).name;
                    }
            ...
            BasicDialog dlg = bPane.showSmallWindow(SwingUtilities.getWindowAncestor(BatchSubmitPane.this), new DialogActionAdapter() {
                public void doOk() {
                    addFields(list);
    ...
    private void addFields(CheckBoxList list) {
        BatchSubmitPane.KeyColumnTableModel model = (BatchSubmitPane.KeyColumnTableModel) keyColumnValuesTable.getModel();
        java.util.List<KeyColumnNameValue> keyColumnNameValueList = new ArrayList<KeyColumnNameValue>();
        ...
        java.util.List<KeyColumnNameValue> newKeyColumnNameValueList = new ArrayList<KeyColumnNameValue>();
        if (!keyColumnNameValueList.isEmpty()) {
            for (int i = 0; i < selected.length; i++) {
                // Richie:先填上空的.
                newKeyColumnNameValueList.add(new BatchSubmitPane.KeyColumnNameValue(false, (BatchSubmitPane.ColumnName) selected[i], new BatchSubmitPane.ColumnValue(""), false));
            }
        }
        // Richie:初始化
        ...
        for (int i = 0; i < selected.length; i++) {
            ...
            for (int j = 0; j < keyColumnNameValueList.size(); j++) {
                if (ComparatorUtils.equals(selected[i], keyColumnNameValueList.get(j).cn)) {
                    ...
                    returnValue = JOptionPane.showOptionDialog(BatchSubmitPane.this, keyColumnNameValueList.get(j).cn.name
    ...
    private void checkTableModel(int returnValue, int coverNumber, BatchSubmitPane.KeyColumnTableModel model, Object[] selected, java.util.List<KeyColumnNameValue> keyColumnNameValueList, java.util.List<KeyColumnNameValue> newKeyColumnNameValueList) {
        if (returnValue == 0) {
            model.removeAllKeyColumnNameValue();
        ...
        if (keyColumnNameValueList.isEmpty()) {
            model.removeAllKeyColumnNameValue();
            for (int i = 0; i < selected.length; i++) {
                model.addKeyColumnNameValue(new BatchSubmitPane.KeyColumnNameValue(false, (BatchSubmitPane.ColumnName) selected[i], new BatchSubmitPane.ColumnValue(""), false));
            }
        }
    ...
    protected class AddFieldAction extends UpdateAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            model.addKeyColumnNameValue(new BatchSubmitPane.KeyColumnNameValue(false, new BatchSubmitPane.ColumnName(""), new BatchSubmitPane.ColumnValue(""), false));
            updateUpdateCheckBoxEnable();
            model.fireTableDataChanged();
    ...
    protected class RemoveFieldAction extends UpdateAction {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            int returnVal = JOptionPane.showConfirmDialog(SwingUtilities.getWindowAncestor(BatchSubmitPane.this),
                    Inter.getLocText("FR-Base_sure_remove_item") + "?", Inter.getLocText("FR-Base_Remove"), JOptionPane.OK_CANCEL_OPTION,
                    JOptionPane.QUESTION_MESSAGE);
            if (returnVal == JOptionPane.OK_OPTION) {
                BatchSubmitPane.KeyColumnTableModel keyColumnNameValueTableModel = (BatchSubmitPane.KeyColumnTableModel) keyColumnValuesTable.getModel();
    ...
    private void refreshColumnsComboBox() {
        ...
        BatchSubmitPane.ColumnName[] columnNames = currentColumnNames();
    ...
    // 得到当前的ColumnName[]
    private BatchSubmitPane.ColumnName[] currentColumnNames() {
        // ben:清除原有的
        ...
        currentColumnNames = new BatchSubmitPane.ColumnName[len];
        for (int i = 0; i < len; i++) {
            currentColumnNames[i] = new BatchSubmitPane.ColumnName(colNames[i]);
        }
        if (currentColumnNames == null) {
            currentColumnNames = new BatchSubmitPane.ColumnName[0];
        }
    ...
    private void initJTableColumn() {
        ...
        column2.setCellRenderer(new BatchSubmitPane.ColumnValueTableCellRenderer());
        ...
        columnsComboBox.setRenderer(new UIComboBoxRenderer() {
            ...
            public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                ...
                if (value instanceof BatchSubmitPane.ColumnName) {
                    this.setText(((BatchSubmitPane.ColumnName) value).name);
                }
        ...
        //设置Column 2的Editor
        column2.setCellEditor(new BatchSubmitPane.ColumnValueEditor());
    }

    @Override
    public void populateBean(SubmitMain submitMain) {
        if (submitMain == null) {
            submitMain = new SubmitMain();
        }
        subMitName = submitMain.getName();
        DMLConfig dmlConfig = submitMain.getDmlConfig();
        ...
        chooseTable.populateBean(new DataBaseItems(submitMain.getDBName(), schema, tableName));
    ...
    private void populateKeyColumnValueTable(DMLConfig dmlConfig) {
        BatchSubmitPane.KeyColumnTableModel keyColumnNameValueTableModel = (BatchSubmitPane.KeyColumnTableModel) this.keyColumnValuesTable.getModel();
        ...
        if (dmlConfig != null) {
            ...
            for (int i = 0; i < dmlConfig.getColumnConfigCount(); i++) {
                ...
                BatchSubmitPane.KeyColumnNameValue newColumnNameValue = new BatchSubmitPane.KeyColumnNameValue(column.isKey(), new BatchSubmitPane.ColumnName(columnName), new BatchSubmitPane.ColumnValue(column.getColumnValue()),
    ...
    @Override
    public SubmitMain updateBean() {
        DataBaseItems para = chooseTable.updateBean(true);
        SubmitMain submitMain = new SubmitMain();
        submitMain.setName(subMitName);
        submitMain.setDBName(para.getDatabaseName());
        ...
        submitMain.setDmlConfig(dmlConfig);
        ...
        for (int i = 0; i < rowCount; i++) {
            BatchSubmitPane.KeyColumnNameValue newKeyColumnNameValue = keyColumnNameValueTableModel.keyColumnNameValueList.get(i);
    ...
    protected class ColumnValueEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        private BatchSubmitPane.ValuePane vPane;
        ...
        protected ColumnValueEditor(Editor<?>[] types) {
            ...
            vPane = new BatchSubmitPane.ValuePane(types);
        ...
        public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
            if (value instanceof BatchSubmitPane.ColumnValue) {
                vPane.populateBean(((BatchSubmitPane.ColumnValue) value).obj);

                if (((BatchSubmitPane.ColumnValue) value).obj != null) {
                    textLabel.setText(((BatchSubmitPane.ColumnValue) value).obj.toString());
                } else {
                    textLabel.setText("");
                }
            }
        ...
        public Object getCellEditorValue() {
            return new BatchSubmitPane.ColumnValue(vPane.updateBean());
        }
    }
    ...
    public static class ColumnName {
        ...
        public boolean equals(Object obj) {
            if (!(obj instanceof BatchSubmitPane.ColumnName)) {
                return false;
            }

            return ComparatorUtils.equals(this.name, ((BatchSubmitPane.ColumnName) obj).name);
        }
    }
    ...
    public static class ColumnValue {
        ...
        public boolean equals(Object obj) {
            if (!(obj instanceof BatchSubmitPane.ColumnValue)) {
                return false;
            }

            return ComparatorUtils.equals(this.obj, ((BatchSubmitPane.ColumnValue) obj).obj);
        }
    }
    ...
    protected static class KeyColumnNameValueTable extends JTable {
        ...
        public KeyColumnNameValueTable() {
            super(new BatchSubmitPane.KeyColumnValueTableModel(null));
        }


        public BatchSubmitPane.KeyColumnTableModel getTableModel4SmartAddCell() {
            return new BatchSubmitPane.KeyColumnValueTableModel((BatchSubmitPane.KeyColumnTableModel) this.getModel());
        }
    ...
    protected abstract static class KeyColumnTableModel extends AbstractTableModel {
        ...
        public KeyColumnTableModel(BatchSubmitPane.KeyColumnTableModel model) {
            if (model != null) {
        ...
        public void addKeyColumnNameValue(BatchSubmitPane.KeyColumnNameValue keyColumnNameValue) {
        ...
        public BatchSubmitPane.KeyColumnNameValue getKeyColumnNameValue(int index) {
    ...
    protected static class KeyColumnValueTableModel extends BatchSubmitPane.KeyColumnTableModel {

        public KeyColumnValueTableModel(BatchSubmitPane.KeyColumnTableModel model) {
            super(model);
        }

        public Object getValueAt(int row, int col) {
            BatchSubmitPane.KeyColumnNameValue knv = keyColumnNameValueList.get(row);
        ...
        public void setValueAt(Object value, int row, int col) {
            ...
            } else if (col == 1 && value instanceof BatchSubmitPane.ColumnName) {
                knv.cn = (BatchSubmitPane.ColumnName) value;
            } else if (col == 2 && value instanceof BatchSubmitPane.ColumnValue) {
                knv.cv = (BatchSubmitPane.ColumnValue) value;
            }
        }
        ...
        public Class<?> getColumnClass(int c) {
            switch (c) {
                ...
                case 1:
                    return BatchSubmitPane.ColumnName.class;
                case 2:
                    return BatchSubmitPane.ColumnValue.class;
            }
            return String.class;
        }
    ...
    public static class KeyColumnNameValue {
        ...
        private BatchSubmitPane.ColumnName cn;
        public BatchSubmitPane.ColumnValue cv;

        public KeyColumnNameValue(boolean isKey, BatchSubmitPane.ColumnName cn, BatchSubmitPane.ColumnValue cv, boolean skip) {
            this.isKey = isKey;
            this.cn = cn;
            this.cv = cv;
        }
    ...
    private class ColumnValueTableCellRenderer extends DefaultTableCellRenderer {
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            if (value instanceof BatchSubmitPane.ColumnValue) {
                if (((BatchSubmitPane.ColumnValue) value).obj != null) {
                    if (((BatchSubmitPane.ColumnValue) value).obj instanceof Date) {
                        this.setText(DateUtils.DATEFORMAT2.format(((BatchSubmitPane.ColumnValue) value).obj));
                    } else {
